...
export default function AlertForm({ tenders }: AlertFormProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!resp.ok || data.error) throw new Error(data.error ?? "Errore nell'invio");
      setStatus("sent");
    } catch (err) {
      setStatus("error");
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      ...
      {isOpen && (
        <div className="px-4 pb-4 border-t border-gray-100 pt-4">
          {tenders.length === 0 ? (
            <p className="text-sm text-gray-500">Esegui prima una ricerca per avere bandi da inviare.</p>
          ) : status === "sent" ? (
            <div className="flex items-center gap-2 text-green-600">
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
              <span className="text-sm font-medium">Report inviato a {email}!</span>
            </div>
          ) : (
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                ...
                className="bg-slate-700 hover:bg-slate-800 disabled:bg-slate-400 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors whitespace-nowrap"
              >
          ...
          <p className="text-xs text-gray-400 mt-2">
            Il report include titolo, compatibilità, importo e link diretto per ogni bando trovato.
          </p>
        </div>
      )}
    </div>
